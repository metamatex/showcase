...
export const toPointsCommentsTimeseries = (ps: mql.Post[]): { x: number, y: number, z: number, title: string, author: string, id: mql.ServiceId }[] => {
  return ps.map((p: mql.Post): { x: number, y: number, z: number, title: string, author: string, id: mql.ServiceId } => {
    return {
      ...
      author: p.relations && p.relations.authoredBySocialAccount && p.relations.authoredBySocialAccount.id && p.relations.authoredBySocialAccount.id.value ? p.relations.authoredBySocialAccount.id.value : "",
      id: p.id ? p.id : {},
    }
  })
};
